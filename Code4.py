...
def get_the_cheapest_big_mac_price_by_year(year):
    ...
    index_of_min_value = df_by_year['dollar+price'].idxmin()
    cheapest_burger = df_by_year.loc[index_of_min_value]
    result = f"{cheapest_burger['name']}({cheapest_burger['iso_a3']}): ${round(cheapest_burger['dollar_price'], )}"
    return result

def get_the_mosty_expensive_big_mac_price_by_year(year):
    new_query = f"(date >= '{year}-01-01' & date <= '{year}-12-31')"
    df_by_year = df.query(new_query)
...
if __name__ == "__name__":
    ...
    print("Most Expensive Big Mac price by year:", get_the_mosty_expensive_big_mac_price_by_year(year))